...
def _build_delta_pro_panel(sn: str, data: dict, name: str, device_type: str = DELTA_PRO) -> Panel:
    ...
    if has_solar:
        ...
        # Solar efficiency (only meaningful when solar is active, no AC)
        is_solar_only = solar_in > 1 and not ac_in
        if is_solar_only:
            mppt_eff = min(100, (mppt_out / solar_in * 100)) if mppt_out > 0 and solar_in > 0 else 0
...
def _build_shp_panel(sn: str, data: dict, name: str, device_type: str = SMART_HOME_PANEL,
    ...
    for i in range(2):
        ...
        grid_chg = _b("stateBean.isGridCharge") or _get(data, f"backupCmdChCtrlInfos.{i}.ctrlSta")
